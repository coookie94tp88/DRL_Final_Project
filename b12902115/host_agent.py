...
from obs_encoding import SEQ_LEN
...
class TrainedHostAgent:
    def __init__(
        ...
        self.rdqn.load_state_dict(state_dict, strict=False)
        self.rdqn.eval()

        print(
            f"✅ HostAgent loaded (Episode {checkpoint.get('episode', 'Unknown')}, "
            f"Doors={self.num_doors}, Device={self.device})"
        )
    ...
    def _process_obs(self, env):
        ...
        hist_tensor = torch.as_tensor(
            host_obs["history"], dtype=torch.float32, device=self.device
        ).unsqueeze(0)
        priv_hist_tensor = torch.as_tensor(
            host_obs["private_honesty_hist"], dtype=torch.float32, device=self.device
        ).unsqueeze(0)
        return curr_tensor, hist_tensor, priv_hist_tensor

    def get_action(self, env):
        curr_tensor, hist_tensor, priv_hist_tensor = self._process_obs(env)
        with torch.no_grad():
            q_pub, q_privs = self.rdqn(curr_tensor, hist_tensor, priv_hist_tensor)
